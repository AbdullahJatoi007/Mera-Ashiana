    ...
    dio.interceptors.add(
      QueuedInterceptorsWrapper(
        onRequest: (options, handler) async {
          ...
          debugPrint(
            '🔑 [API TOKEN ATTACHED?] ${token != null ? 'YES' : 'NO'}',
          );
        ...
        onError: (DioException e, handler) async {
          ...
          // A 401 on an auth route means "bad credentials", not "stale session".
          // Don't try to refresh/logout for login, register, google, or send-otp.
          final isAuthRoute = e.requestOptions.path.contains(
          ...
          if (e.response?.statusCode == 401 && !isAuthRoute) {
            ...
            try {
              ...
              if (success) {
                debugPrint(
                  '✅ [API REFRESH] Success! Retrying original request.',
                );
